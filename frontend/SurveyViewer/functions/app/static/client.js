...
const showOpenSurvey = async (survey) => {    
  ...
  const form = createElement('form', null, surveyElement, [{ name: 'method', value: 'POST'}, { name: 'action', value: './submit'}]);
  
  const csrf = document.querySelector('#csrf');
  form.appendChild(csrf.cloneNode(true));
  createElement('input', null, form, [{name: 'type', value: 'hidden'}, {name: 'value', value: survey.participantId}, {name: 'id', value: 'participantId'}, {name: 'name', value: 'participantId'}])

  survey.questions.forEach(({ id, text, choices }) => {
    createElement('label', text, form, [{name: 'for', value: id}]);
    const select = createElement('select', null, form, [{name: 'id', value: id}, {name: 'name', value: id}, {name: 'required', value: 'required'}]);
    choices.forEach(({ isSelected, text: choiceText, value }) => {
      createElement('option', choiceText, select, [{name: 'value', value: value}, {name: 'selected', value: isSelected}]);
    });    
  });

  createElement('input', null, form, [{name: 'type', value: 'submit'}, {name: 'value', value: 'Abstimmen'}])
};
